...
\label{sec:dev}
After every choice was fixed on the training split, we ran the frozen system once on the
development split (519 queries, 1,214 steps). % phase8_v1_dev
Table~\ref{tab:dev} shows each stage. Every stage improves on the previous one, and the
direction and order of the gains match training: dense retrieval $+0.1857$
[$+0.1605$, $+0.2112$], step fusion $+0.0080$ [$+0.0011$, $+0.0158$], reranking $+0.0673$
[$+0.0488$, $+0.0868$]. Development scores are not lower than training scores, so we see no sign
of overfitting to the training queries. On 1b the system scores 0.3147 against BM25's 0.1121.
Reranking lowers two of 13 domains on development data (iota, 3 queries; monero, 19) and raises
History, 240 of the 519 queries, from 0.3035 to 0.3810.

\begin{table}[t]
\centering\small
\begin{tabular}{lr}
\toprule
System (1a, dev) & nDCG@10 \\
\midrule
Official BM25 & 0.1055 \\
Dense (Diver-0.6B) & 0.2912 \\
+ step fusion & 0.2992 \\
+ ReasonRank-7B, top 30 & 0.3665 \\
\midrule
1b: dense step rankings & 0.3147 \\
\bottomrule
\end{tabular}
\caption{The single development-split evaluation of the frozen system.}
\label{tab:dev}
\end{table}

\section{Compute}
All GPU work ran on free Kaggle T4 GPUs (16\,GB, no bf16, no FlashAttention). Corpus embedding:
6.0 hours on two GPUs. Reranking: 29.7--33 seconds per query on two GPUs with vLLM (7.6 hours for
896 training queries, 4.75 hours for 519 development queries); prompts average 6.5k tokens per window, and the run is bound by prompt processing.
