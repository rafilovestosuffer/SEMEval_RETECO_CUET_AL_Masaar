"""Frozen pipeline, stage 3 of 3 — ReasonRank-7B over the fused top-30 of any split.
...
GPU kernel, T4 x2. Input is a private dataset (``reteco-pipeline-input``) holding two things:
...
REPORT = WORKING / "pipeline_rerank_report.json"
...
def find_input() -> tuple[Path, Path]:
    ...
    if not pkg:
        die(f"no reteco/rerank.py under {KAGGLE_INPUT}; attach the reteco-pipeline-input dataset")
    if not runs:
