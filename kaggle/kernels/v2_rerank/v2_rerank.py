"""v2 experiment kernel — ReasonRank-7B over the top-30 of 1a (fused) or 1b (dense step) runs.

This is `kaggle/kernels/phase7_rerank/phase7_rerank.py`, the kernel that measured H4 on
train, with the split made a constant and the input dataset renamed. Everything that shapes
the model input or the output order is unchanged. Stage 1 is `pipeline_dense`, stage 2 is
`eval/write_fused_runs.py`. On test a partial run is not acceptable (a missing topic scores
0): the RESULT section says loudly when topics were not reranked, and those must be filled
from the fused run before submitting (`submit/assemble_runs.py`).

The Phase 7 docstring follows unchanged, except that the split is no longer train-only.

v2 step 2 (reports/RETECO v2 improvement levers.md): does reranking the 1b step lists help? For
1b the reranker query is the official step template (query + "Step: " + instruction), the same
text the dense 1b run was retrieved with. Otherwise identical to pipeline_rerank.

GPU kernel, T4 x2. Input is a private dataset (``reteco-v2-input``) holding two things:
...
TAG = "cuet_reasonrank7b_" + SUBTRACK

OUT_RUNS = WORKING / "runs"
RAW = WORKING / "raw_outputs.jsonl"
REPORT = WORKING / "v2_rerank_report.json"
...
def find_input() -> tuple[Path, Path]:
    ...
    runs = sorted(KAGGLE_INPUT.glob(f"**/runs/*/run_{SUBTRACK}_{SPLIT}.trec"))
    if not (pkg and runs):
        ...
        runs = sorted(unpacked.glob(f"**/runs/*/run_{SUBTRACK}_{SPLIT}.trec")) or sorted(
            unpacked.glob(f"**/*/run_{SUBTRACK}_{SPLIT}.trec"))
    ...
    if not pkg:
        die(f"no reteco/rerank.py under {KAGGLE_INPUT}; attach the reteco-v2-input dataset")
    if not runs:
...
def load_candidates(runs_root: Path, read_run) -> dict[str, dict[str, list[str]]]:
    return {d.name: read_run(d / f"run_{SUBTRACK}_{SPLIT}.trec")
            for d in sorted(runs_root.iterdir()) if (d / f"run_{SUBTRACK}_{SPLIT}.trec").is_file()}
...
def main() -> int:
    ...
    for d in domains:
        if SUBTRACK == "1a":
            ...
            for rec in read_jsonl(DATA_DIR / TRACK / d / f"steps_{SPLIT}.jsonl"):
                for st in rec.get("steps", []):
                    if st["step_id"] in candidates[d]:
                        queries[st["step_id"]] = STEP_TEMPLATE.format(
                            query=rec["query"], step_instruction=st["step_instruction"])
    ...
    if SUBTRACK == "1b" and SAMPLE_PARENTS:
        parents = sorted(set(parent_of_topic.values()))
        random.Random(SEED).shuffle(parents)
        keep = set(parents[:SAMPLE_PARENTS])
        order = [t for t in order if parent_of_topic[t] in keep]
        report["parents_sampled"] = len(keep)
    if LIMIT:
        order = order[:LIMIT]
